...
export const ParticleBackground = ({ className, children }: ParticleBackgroundProps) => {
    ...
    useEffect(() => {
        ...
        const particleCount = 180; // Triple density for a 'Starfield' effect
        const connectionDistance = 120; // Tightened connection logic
        ...
        const init = () => {
            ...
            for (let i = 0; i < particleCount; i++) {
                particles.push({
                    ...
                    vx: (Math.random() - 0.5) * 0.3, // Slower, celestial drift
        ...
        const animate = () => {
            ...
            particles.forEach((p, i) => {
                ...
                // Draw Particle (Static twinkle feel)
                ctx.beginPath();
                ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2);
                ctx.fillStyle = `rgba(255, 255, 255, ${0.4 + Math.random() * 0.2})`;
                ctx.fill();

                // Proximity Webbing (Purely Autonomous)
                for (let j = i + 1; j < particles.length; j++) {
                    ...
                    if (dist < connectionDistance) {
                        drawLine(p, p2, 0.15 * (1 - dist / connectionDistance));
                    }
                }
            });
